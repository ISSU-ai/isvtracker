...
Apps Script 코드를 수정한 경우에는 기존 배포의 편집(연필) 메뉴에서 새 버전을 배포해야 변경 내용이 실제 웹앱에 적용됩니다. 기존 배포를 편집해 새 버전을 지정하면 프론트엔드에 설정한 `/exec` URL을 유지할 수 있습니다. 현재 저장소의 파트너 등록·업무별 진행 입력 기능은 아래 기존 웹앱에 버전 5로 배포되어 있습니다.
...
현재 웹앱 설정은 실행 사용자 권한 및 조직 계정 접근입니다. 코드의 허용 계정 목록에 포함된 사용자만 Sheet 데이터를 읽거나 저장할 수 있습니다.
